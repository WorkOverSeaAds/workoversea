...
tags:
  ...
  - react
  - Hooks
description: 了解 react 中 Hooks 中的使用
---
...
整理自：https://zh-hans.reactjs.org/docs/hooks-rules.html
...
setCount函数用于更新state，它接收一个新的值，并将组件的一次新渲染加入队列。在后续的重新渲染中，`useState` 返回的第一个值将始终是更新后最新的 state。

>   React 会确保 setCount 函数的标识是稳定的，并且不会在组件重新渲染时发生变化。这就是为什么可以安全地从 `useEffect` 或 `useCallback` 的依赖列表中省略 setCount。
...
[hook API介绍 react 官网](https://zh-hans.reactjs.org/docs/hooks-reference.html)
