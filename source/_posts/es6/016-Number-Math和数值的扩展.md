...
description: 了解 Number Math
---


# Number-Math和数值的扩展

整理转载自：https://es6.ruanyifeng.com/#docs/number
...
### 7.2 Math.sign()判断是正数、负数、还是零

`Math.sign`方法用来判断一个数到底是正数、负数、还是零。对于非数值，会先将其转换为数值。对于那些无法转为数值的值，会返回`NaN`。
...
`Math.expm1(x)`返回 $ {x^2} $ - 1，即`Math.exp(x) - 1`。
